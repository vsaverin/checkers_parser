...
class GitLabArtifactsDownloader:
    ...
    def download_artifacts(self):
        job_url = f'{self.gitlab_url}/api/v4/projects/{self.project_id}/jobs?scope=success&per_page=1&name={self.job_name}'
        headers = {'Private-Token': self.access_token}
        response = requests.get(job_url, headers=headers)

        if response.status_code == 200:
            job_id = response.json()[0]['id']
            artifacts_url = f'{self.gitlab_url}/api/v4/projects/{self.project_id}/jobs/{job_id}/artifacts'
            artifacts_response = requests.get(artifacts_url, headers=headers, allow_redirects=True)
